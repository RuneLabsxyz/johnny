...
export const bid = (chain: StarknetChain) => action({
    ...
    async handler(data, ctx, agent) {
        ...
        let estark_contract = new Contract(estark_abi, estark_address, chain.provider);
        let token_contract = new Contract(token_abi, data.token_for_sale, chain.provider);
        ...
        let sale_approve_call: Call = {contractAddress: data.token_for_sale, entrypoint: "approve", calldata: CallData.compile({spender: ponziland_address, amount: cairo.uint256(price * BigInt(5))})};
